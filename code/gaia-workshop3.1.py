...
def main():
    	global text, new_text,dev, ph, timestamp, current, power_consumption	
   	time.sleep(1)
    	led=[0,0,0]
   	led_part=[0,0,0]
        t=0
		...
		if t==0:
			...
			if new_t != t:
				...
				print strtime
 	  	     		...
 	  	     		for i in [0, 1, 2]:			
	        			print phases[i]["uri"], " Current: " +str(float("{0:.2f}".format(current[i][t-1]/1000))) +" A  "+ "Power: "+  str(float("{0:.2f}".format(power_consumption[i][t-1]*230/1000))) + " W" 
